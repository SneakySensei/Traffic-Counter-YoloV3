...
from numpy import expand_dims
def load_image_pixels(rgbimg, shape):
 # load the image to get its shape
    image = rgbimg
    width, height = cap.get(cv2.CAP_PROP_FRAME_WIDTH), cap.get(cv2.CAP_PROP_FRAME_HEIGHT)

    # load the image with the required size
    image = cv2.resize(rgbimg, shape)
...
def draw_boxes(rgbimg, v_boxes, v_labels, v_scores):
    # plot each box
    for i in range(len(v_boxes)):
        ...
        cv2.putText(img, v_labels[i], (x1+15,y1-15), cv2.FONT_HERSHEY_SIMPLEX,1,color,1)
...
while True:
    ...
    image, image_w, image_h = load_image_pixels(rgbimg, (net_w, net_w))
    ...
    prefLabels = ["person", "bicycle", "car", "motorbike", "bus", "truck", "cat", "dog", "horse", "cow"]
    ...
    for i in range(len(v_boxes)):
        ...
        if v_labels[i] in prefLabels:
            trafficCount+=1
    countList.append(trafficCount)
    frames+=1
    if frames==5:
        frames = 0
        trafficCount = statistics.median(countList)
        countList=[]
        print(trafficCount)
    
        # # Post the count to backend
        # data = {'postid': 102, 'count':trafficCount}
        # r = requests.post("http://192.168.225.25:9000/opencv", data = data)


    # draw what we found
    draw_boxes(rgbimg, v_boxes, v_labels, v_scores)
